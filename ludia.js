...
(function () {
  ...
  function statusText(g,s) { const end=g.status(s); if(end.ended){ if(!end.winner)return t('draw'); if(active?.options.partner!=='local')return t(end.winner===1?'youWin':'ludiaWin'); return t(end.winner===1?'youWin':'otherWin'); } return isComputer(s)?t('thinking'):t('turn',{name:name(s.turn)}); }
  function rulesHelp(g) { return `<details class="match-help"><summary>${esc(t('helpRules'))}</summary><p>${esc(t({ 'tic-tac-toe':'helpTicTacToe','connect-four':'helpConnectFour',battleship:'helpBattleship',sudoku:'helpSudoku',tetris:'helpTetris',domino:'helpDomino',checkers:'helpCheckers' }[g.id]||'helpRulesText'))}</p></details>`; }
  function match(g) {
    if(!restore(g)) return setup(g);
    const s=active.states.at(-1), end=g.status(s);
    return `${back(g.id)}${phaseNav(g,'play')}<div class="match-heading"><h1>${esc(tr(g.title))}</h1><span class="pace-pill">${esc(t('pace'))}</span></div><div class="match-layout"><section class="match-board">${renderBoard(g,s,!end.ended&&!isComputer(s))}</section><aside class="match-panel"><div class="turn-marker ${end.ended?'is-success':''}" role="status">${esc(window.LudiaStatus?.[g.id]?.(s,api)||statusText(g,s))}</div>${window.LudiaPanels?.[g.id]?.(s,api)||''}${rulesHelp(g)}<p class="match-message" role="status">${esc(message)}</p><div class="match-tools">${button(t('hint'),'hint','secondary',end.ended||isComputer(s)||window.LudiaPrivate?.[g.id]?.(s,api)?'disabled':'')}${button(t('undo'),'undo','secondary',active.actions.length?'':'disabled')}${button(t('restart'),'new','secondary')}</div><p class="save-note">${esc(t('saved'))}</p></aside></div>`;
  }
